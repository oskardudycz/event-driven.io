...
If we add to that distributed locking or [detecting conflicting checkpointing](/pl/checkpointing_message_processing/) to ensure that there's only one worker instance handling messages for the processor and partition id, then this should be good enough for the majority of cases.
