...
How to find such scenarios? [Doing Example Mapping during modelling can be a good option for that](/pl/intro_to_example_mapping/).
...
In our case, it'd be better to have a coordinator ([workflow](/pl/how_to_have_fun_with_typescript_and_workflow/), [saga, process manager](/pl/saga_process_manager_distributed_transactions/), etc.) that publishes the _OrderConfirmed_ event for modules not on the critical path and sends commands like _RecordPayment_ and _InitiateShipment_.
...
Lacking tracing and observability of the business process is one of the most common issues [I see in my clients' projects](/pl/training/). As said, if we don't want to end up with parliament instead of proper communication in our system, we need to be explicit about our intention.
...
It's a common anti-pattern, a variation of [State Obsession](/pl/state-obsession/). Too many people believe it's fine to connect the messaging system to the database, use tools like [Change Data Capture](https://en.wikipedia.org/wiki/Change_data_capture), and publish it automatically to others.
...
You not only deal with [Clickbait Events](/pl/clickbait_event/) but also have a leaking business abstraction. All consumers need to understand the internals of your processing to detect a specific type of change. I wrote about it in detail in [Internal and external events, or how to design event-driven API](/pl/internal_external_events/).
...
**See also more in series about [event modelling anti-patterns](/pl/anti-patterns/):**
...
- [Passive Aggressive Events](/pl/passive_aggressive_events),
