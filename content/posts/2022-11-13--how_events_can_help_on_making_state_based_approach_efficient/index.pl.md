...
MongoDB has a sweet set of atomic operations you can apply to the document. You can update a subset of properties, increment values, and nested arrays. **You can do upserts and modify the whole document.** This is not so common for [key-value databases](/pl/key-value-stores/). It's fine to keep documents a bit bigger, denormalised and use the same document for writing and read models by just querying for a subset of information. We're not getting a big penalty hit if we're doing atomic operations. Having that, loading and updating the whole document each time is overkill.
