...
**That is why tools such as EventStorming, Event Modeling and other _sticky-notes-based_ tools are so popular.** They allow us to throw our ideas onto the wall, crunch them and stimulate discussions. 
...
**The brainstorming itself may concern specific functionalities as well as the global vision of the system.** EventStorming divides this into _"big picture"_ and _"process/design level"_. First, we look at the events that are essential from the overall system perspective, then we go lower (read more about the distinction between public and private events on my blog: ["Events should be as small as possible, right?"](/en/events_should_be_as_small_as_possible/)).
...
Common sense and adapting the tools to my needs are important for me. I do sketches on paper. Instead of specific methodologies, I like to create blocks in an application like [draw.io](https://www.drawio.com/). They may not be the most maintainable in the long term, but they can provide a good entry point for discussion or [immutable decision record](/en/how_to_successfully_do_documentation_without_maintenance_burden/).
