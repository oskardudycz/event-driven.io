...
Of course, we should be careful and not try to send _passive-aggressive events_, so events that should be commands. If we know that we'll always have a single consumer for an event that always needs to run the specific logic and expect to get the particular event back, then it should probably be a command. Read more in [What's the difference between a command and an event?](/pl/whats_the_difference_between_event_and_command/).
