...
In the last article, I explained [how to rebuild Event-Driven Read Models in a safe and resilient way](/pl/rebuilding_event_driven_read_models/). I asked readers to let me know if they find any blind spots in my design.
...
Let me recap the situation described in [the previous article](/pl/rebuilding_event_driven_read_models/). We're using PostgreSQL to store events and read models. We want to rebuild an inline projection, one that's applied in the same transaction as the appended event.
...
We could handwave it with [it won't ever happen!](/pl/no_it_can_never_happen/). But, well, under load, this will happen eventually. You might not even notice it, but it will. The timing window is small, but with enough throughput, it becomes a certainty.
...
I wrote about this exact problem in [How Postgres sequences issues can impact your messaging guarantees](/pl/ordering_in_postgres_outbox/). The same visibility challenges that affect outbox ordering apply here. Transaction IDs are useful for ordering within committed data, but they can't help us coordinate with transactions that haven't committed yet.
...
- Global position sequencing (with all the transaction visibility handling from the [outbox ordering article](/pl/ordering_in_postgres_outbox/))
...
- [Rebuilding Event-Driven Read Models in a safe and resilient way](/pl/rebuilding_event_driven_read_models/)
